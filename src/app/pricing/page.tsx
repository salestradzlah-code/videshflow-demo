...
import { futureAddonConfigs, getAddonFlags, getPaidProductRuntime } from "@/lib/paidProducts";
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-8 rounded-xl border border-zinc-200/80 bg-white p-6 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-zinc-500">Optional add-ons</p>
              <h2 className="mt-2 text-xl font-semibold text-zinc-900">Prepared add-on modules</h2>
              <p className="mt-2 max-w-3xl text-sm leading-6 text-zinc-600">
                These add-ons are configured as future Stripe products, but bundled checkout is not active in this version. Premium already includes persona modules; standalone add-on checkout will stay paused until a safe bundled purchase flow is implemented and tested.
              </p>
            </div>
            <span className="rounded-full bg-amber-50 px-3 py-1 text-xs font-semibold text-amber-700">
              {addonFlags.checkoutEnabled ? "Checkout flag on — bundled checkout still not exposed" : "Checkout off"}
            </span>
          </div>
          <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
            {futureAddonConfigs.map((addon) => (
              <div key={addon.slug} className="rounded-xl border border-zinc-200 bg-zinc-50 p-4">
                <p className="text-sm font-semibold text-zinc-900">{addon.title}</p>
                <p className="mt-1 text-xs font-semibold uppercase tracking-[0.12em] text-zinc-500">{addon.priceLabel}</p>
                <p className="mt-2 text-xs leading-5 text-zinc-600">{addon.description}</p>
                <p className="mt-3 rounded-lg bg-white px-3 py-2 text-[11px] leading-5 text-zinc-500">
                  Prepared only. No standalone add-on checkout today.
                </p>
              </div>
            ))}
          </div>
