...
export function Header() {
  ...
  return (
    <>
      ...
      {/*
        Mobile nav drawer — rendered OUTSIDE <header> so it is in the root stacking context.
        The header uses backdrop-blur which creates a CSS filter stacking context; any
        `position: fixed` children inside it are trapped within that context and cannot
        reliably cover sibling elements (e.g. the trust bar div in layout.tsx).
        Moving it here gives it a true global z-[200] that covers header, trust bar, and hero.
      */}
      <div
        className={`fixed inset-0 z-[200] transition-opacity duration-200 ease-in-out xl:hidden ${
      ...
      >
        ...
        <div
          ...
          className={`absolute right-0 top-0 flex h-full w-full max-w-sm flex-col bg-white shadow-2xl transition-transform duration-200 ease-in-out ${
        ...
        >
          ...
          <nav className="flex flex-1 flex-col gap-1 overflow-y-auto px-4 py-4">
            {navItems.map((item) => (
              <Link
                ...
                href={item.href}
                onClick={() => setMenuOpen(false)}
                className="rounded-xl px-4 py-3.5 text-base font-semibold text-zinc-800 transition-colors duration-200 ease-in-out hover:bg-zinc-50 hover:text-emerald-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-600 focus-visible:ring-offset-2"
              >
                {item.label}
              </Link>
            ))}
          ...
          <div className="border-t border-zinc-200/80 px-5 py-5">
            ...
            <p className="mt-3 text-center text-xs text-zinc-400">Planning support only \u2014 not professional advice.</p>
          </div>
        </div>
      </div>
    </>
